...
from zarr.core.chunk_grids import RegularChunkGrid, RegularChunkGridConfiguration
...
class ArrayV3Metadata(ArrayMetadata):
    ...
    chunk_grid: RegularChunkGrid
    chunk_key_encoding: CHUNK_KEY_ENCODINGS
    fill_value: Any
    codecs: tuple[Codec, ...]
    attributes: dict[str, Any] = field(default_factory=dict)
    dimension_names: tuple[str, ...] | None = None
    zarr_format: Literal[3] = field(default=3)
    node_type: Literal["array"] = field(default="array")
    ...
    def __post_init__(
        ...
        """
        # shape_parsed = parse_shapelike(self.shape)
        # data_type_parsed = parse_dtype(self.data_type)
        # chunk_grid_parsed = ChunkGrid.from_dict(self.chunk_grid)
        # chunk_key_encoding_parsed = ChunkKeyEncoding.from_dict(self.chunk_key_encoding)
        # dimension_names_parsed = parse_dimension_names(self.dimension_names)
        # fill_value_parsed = parse_fill_value_v3(self.fill_value, dtype=data_type_parsed)
        # attributes_parsed = parse_attributes(self.attributes)
        # codecs_parsed_partial = parse_codecs(self.codecs)
...
class ArrayV2Metadata(ArrayMetadata):
    ...
    def __init__(
        ...
        *,
        ...
        object.__setattr__(
            ...
            "chunk_grid",
            RegularChunkGrid(
                configuration=RegularChunkGridConfiguration(chunk_shape=chunks_parsed)
            ),
        )
    ...
    def chunks(self) -> ChunkCoords:
        return self.chunk_grid.configuration.chunk_shape
    ...
    def to_dict(self) -> JSON:
        ...
        zarray_dict["chunks"] = self.chunk_grid.configuration.chunk_shape
