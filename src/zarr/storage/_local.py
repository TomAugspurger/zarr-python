...
def _get_into(path: Path, out: Buffer, byte_range: ByteRequest | None) -> int | None:
    if byte_range is not None:
        raise NotImplementedError("byte_range is not supported yet")
    with path.open("rb") as f:
        # TODO: check if there's a way to type this properly.
        return f.readinto(out)  # type: ignore[arg-type]
...
class LocalStore(Store):
    ...
    async def get_into(
        ...
        try:
            return await asyncio.to_thread(_get_into, path, out, byte_range)
        except (FileNotFoundError, IsADirectoryError, NotADirectoryError):
            return None
